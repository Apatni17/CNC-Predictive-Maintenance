...
class CorrectedXGBoostPredictor:
    def __init__(self):
        self.model = None
    ...
    def get_correct_label(self, exp_id):
        """Get the CORRECT label for each experiment"""
        # CORRECTED LABELING:
        # 1-5: unworn
        # 6-10: worn  
        # 11-12: unworn
        # 13-16: worn
        if exp_id in [1, 2, 3, 4, 5, 11, 12]:
            return 0  # unworn
        elif exp_id in [6, 7, 8, 9, 10, 13, 14, 15, 16]:
            return 1  # worn
        else:
            raise ValueError(f"Unknown experiment ID: {exp_id}")
        
    def train_model(self, data_dir="data/CNC mill wear /"):
        """Train the CORRECTED XGBoost model with proper labels"""
        print("🎯 Training CORRECTED XGBoost Tool Wear Predictor...")
        print("🔧 Using CORRECT labeling:")
        print("   Unworn: 1-5, 11-12")
        print("   Worn: 6-10, 13-16")
        ...
        for filename in self.training_files:
            ...
            try:
                ...
                # CORRECTED LABELING
                data['tool_wear'] = self.get_correct_label(exp_num)
                
                label_name = "unworn" if data['tool_wear'].iloc[0] == 0 else "worn"
                print(f"✅ Loaded {filename} (Experiment {exp_num}) - CORRECTED: {label_name}")
        ...
        # Experiment-based train/test split with CORRECTED labels
        random.seed(42)
        unworn_experiments = [1, 2, 3, 4, 5, 11, 12]  # CORRECTED
        worn_experiments = [6, 7, 8, 9, 10, 13, 14, 15, 16]  # CORRECTED
        
        print(f"\n🔧 CORRECTED experiment classification:")
        print(f"   Unworn experiments: {unworn_experiments}")
        print(f"   Worn experiments: {worn_experiments}")
        ...
        print(f"\n📊 CORRECTED Training Data Summary:")
        ...
        train_unworn = sum(train_data['tool_wear'] == 0)
        train_worn = sum(train_data['tool_wear'] == 1)
        test_unworn = sum(test_data['tool_wear'] == 0)
        test_worn = sum(test_data['tool_wear'] == 1)
        
        print(f"\n📈 CORRECTED Train/Test Split:")
        print(f"   Training samples: {len(train_data)} (Unworn: {train_unworn}, Worn: {train_worn})")
        print(f"   Test samples: {len(test_data)} (Unworn: {test_unworn}, Worn: {test_worn})")
        ...
        # Train corrected XGBoost model
        print(f"\n🤖 Training CORRECTED XGBoost Model...")
        ...
        print(f"\n📈 CORRECTED Test Set Performance:")
        ...
        # Save CORRECTED model and components
        joblib.dump(self.model, 'corrected_xgboost_model.pkl')
        joblib.dump(self.available_features, 'corrected_xgboost_features.pkl')
        joblib.dump(self.training_files, 'corrected_training_files.pkl')
        joblib.dump(self.test_accuracy, 'corrected_test_accuracy.pkl')
        joblib.dump({
            'medium_threshold': self.medium_risk_threshold,
            'high_threshold': self.high_risk_threshold
        }, 'corrected_risk_thresholds.pkl')
        
        # Save the correct labeling for reference
        labeling_info = {
            'unworn_experiments': unworn_experiments,
            'worn_experiments': worn_experiments,
            'validation_experiments': [17, 18],
            'validation_labels': {'17': 'unworn', '18': 'worn'}
        }
        joblib.dump(labeling_info, 'corrected_labeling_info.pkl')
    ...
    def predict_tool_wear(self, data):
        ...
        if self.model is None:
            try:
                self.model = joblib.load('corrected_xgboost_model.pkl')
                self.available_features = joblib.load('corrected_xgboost_features.pkl')
                self.test_accuracy = joblib.load('corrected_test_accuracy.pkl')
                thresholds = joblib.load('corrected_risk_thresholds.pkl')
                self.medium_risk_threshold = thresholds['medium_threshold']
                self.high_risk_threshold = thresholds['high_threshold']
                self.is_trained = True
            except FileNotFoundError:
                raise ValueError("Corrected XGBoost model files not found. Please train the model first.")
    ...
    def analyze_machine_health(self, data, wear_probabilities):
        """Analyze machine health using corrected risk assessment"""
        if isinstance(data, str):
            data = pd.read_csv(data)
    ...
    def create_performance_plots(self, y_test, y_test_pred, y_test_proba, feature_importance):
        ...
        axes[0,0].set_title('Confusion Matrix (CORRECTED XGBoost)')
        ...
        axes[0,1].set_title('Feature Importance (CORRECTED XGBoost)')
        ...
        axes[1,0].set_title('Risk Distribution (CORRECTED Labels)')
        
        # 4. Model Summary
        summary_text = f"""
        🎯 CORRECTED XGBoost Predictor
        ...
        axes[1,1].set_title('CORRECTED Model Summary')
        axes[1,1].axis('off')
        
        plt.tight_layout()
        plt.savefig('corrected_xgboost_performance.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("📊 CORRECTED performance plots saved as 'corrected_xgboost_performance.png'")

def main():
    """Train the CORRECTED XGBoost model"""
    print("🎯 CORRECTED XGBoost Tool Wear Predictor")
    print("=" * 50)
    
    # Create predictor
    predictor = CorrectedXGBoostPredictor()
    
    # Train model with CORRECT labels
    test_accuracy, feature_importance = predictor.train_model()
    
    print(f"\n🎉 CORRECTED XGBoost Training Complete!")
    print(f"📊 Test accuracy: {test_accuracy:.3f}")
    print(f"🎯 Ready for ACCURATE testing with experiments 17-18!")
